...
void new_exit(vars_t *vars)
{
	...
	int status;
	/*If exit has arguments, we handle it*/
	if (_strcmpr(vars->array_tokens[0], "exit") ==
			0 &&
		vars->array_tokens[1] != NULL)

	{
		/* With this function we make sure that the entered number is valid */
		status = _atoi(vars->array_tokens[1]);
		/* we handle case of error*/
		if (status == -1)
		{
			vars->status = 2;
 ...
 * @vars: Indicate struct of variables
 * Return: void.
 */
...
void new_setenv(vars_t *vars)
{
	char **key;
	char *var;
	/** if the arguments to setenv are wrong we print the error message **/
...
/**
 * new_unsetenv - To remove an environment variable
 * @vars: Indicate pointer to a struct of variables
 *
 * Return: void
 */
